...
def process_locations(csv_path, n_clusters):
    ...
    # Normalize coordinates into a Colombian bounding box approximation
    ...
    demand_points = df[df['State_of_Charge_%'] < soc_threshold][['lat', 'lon']]

    model = KMeans(n_clusters=n_clusters, random_state=42)
    model.fit(demand_points)
    ...
    for i, point in enumerate(centers):
        folium.Marker(
            ...
            popup=f"Suggested Station {i+1}",
            icon=folium.Icon(color='green', icon='bolt', prefix='fa')
        ).add_to(m)
    return centers.tolist(), m._repr_html_()